...
def run_game(player_name, difficulty):
    ...
    ai_speed = {"Easy": 40, "Medium": 60, "Hard": 100}[difficulty]  
    player_speed = 300  
    ...
    while running:
        ...
        player_label = FONT.render(f"{player_name} Position: {int(player_x) // MOVE_DISTANCE}/{QUESTIONS_TO_WIN}", True, BLACK)
...
def start_game():
    ...
    tk.Button(btn_frame, text="Easy", width=10, font=("Courier", 12),
              command=lambda: on_difficulty_select("Easy")).grid(row=0, column=0, padx=5)
    tk.Button(btn_frame, text="Medium", width=10, font=("Courier", 12),
              command=lambda: on_difficulty_select("Medium")).grid(row=0, column=1, padx=5)
    tk.Button(btn_frame, text="Hard", width=10, font=("Courier", 12),
              command=lambda: on_difficulty_select("Hard")).grid(row=0, column=2, padx=5)
