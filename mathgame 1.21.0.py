...
def main():
    ...
    # Car Size.
    CAR_WIDTH, CAR_HEIGHT = 120, 70
    # Players Car.
    PLAYER_CAR = pygame.image.load("Images/car_player2.png")
    PLAYER_CAR = pygame.transform.scale(PLAYER_CAR, (CAR_WIDTH, CAR_HEIGHT))
    # AI Car.
    AI_CAR = pygame.image.load("Images/car_ai3.png")
    ...
    INPUT_FONT = pygame.font.SysFont("Helvetica", 16)
    ...
    def generate_medium_question():
        ...
        if shape == "square":
            ...
            question = f"Find the area of a square with a length of {side}?"

            # Calculates the area (Using Area of a square side x side).
            answer = side * side
        ...
        # Rectangle.
            ...
            question = f"Find the area of a rectangle with a length of {l} and width of {w}?"

            # Calculates Area (Using formula of rectangle: length × width).
            answer = l * w
        ...
        # Circle.
            ...
            question = f"Find the area of a circle with radius {radius}? (use π = 3.14)"
        ...
        # Triangle.
            ...
            # Generates a random base and height.
            ...
            # Saves shape data to be used for drawing.
            shape_data = ("triangle", (base, height))
    ...
    def generate_hard_question():
        ...
        # Differentiation Questions(in form f(x) = ax^n).
        if qtype == "diff_question":
            # Generates random coefficent and exponents.
            a = random.randint(1, 5)
            n = random.randint(1, 5)
            question = f"Find the derivative of {a}x^{n}?"
        ...
        # Integration Questions(in form: ∫ ax^n dx).
        elif qtype == "int_question":
    ...
    def save_to_leaderboard(name, time_taken, difficulty):
        filename = f"leaderboard_{difficulty.lower()}.txt"
    ...
    def show_leaderboard():
        def open_difficulty_board(difficulty):
            filename = f"leaderboard_{difficulty.lower()}.txt"
            if not os.path.exists(filename):
                messagebox.showinfo("Leaderboard", f"No {difficulty} leaderboard data found.")
                return

            # Read and parse leaderboard entries.
            leaderboard_entries = []
            with open(filename, "r") as file:
                for line in file:
                    try:
                        name, time_str = line.strip().split(":")
                        time_taken = float(time_str.strip().split()[0])
                        leaderboard_entries.append((name.strip(), time_taken))
                    except ValueError:
                        continue
            # Sorts the entries based on time.
            ...
            # Use treeview widget for the leaderboard table.
            columns = ("Name", "Time")
            tree = ttk.Treeview(leaderboard_window, columns=columns, show="headings", height=15)

            # Adds style to the heading(bold).
            style = ttk.Style()
            style.configure("Treeview.Heading", font=("Helvetica", 12, "bold"))
            style.configure("Treeview", font=("Helvetica", 11))

            # Defines headings for the columns.
            tree.heading("Name", text="Name")
            tree.heading("Time", text="Time(seconds)")

            # Centre aligns the entries in the columns.
            tree.column("Name", anchor="center", width=160)
            tree.column("Time", anchor="center", width=140)

            # Enters in the leaderboard data.
        ...
        # Adds buttons for each difficulty.
    ...
    def start_game():
        ...
        if os.path.exists("Images/logo.png"):
            logo_raw = Image.open("Images/logo_option1.png")
            resized_logo = logo_raw.resize((300, 200), Image.Resampling.LANCZOS)
            logo_img = ImageTk.PhotoImage(resized_logo)
            logo_label = tk.Label(root, image=logo_img)
            logo_label.image = logo_img
            logo_label.pack(pady=1)
        else:
            messagebox.showinfo("Error", "logo.png not found.")
