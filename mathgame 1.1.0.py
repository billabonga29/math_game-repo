...
def submt(user_entry):
    ...
    # Blanks
    if user_input == "":
        warning = Label(app, text="NO _____!", fg="RED", font=("Courier", 16))
        warning.place(relx=0.25, rely=0.16)
        
    # Alphabet, Whitespace and Symbols
    elif user_input.isdigit() == False:
        if user_input.isalnum() == True:
            warning = Label(app, text="NO LETTUCE!", fg="RED", font=("Courier", 16))
            warning.place(relx=0.25, rely=0.16)
        elif " " in user_input:
            warning = Label(app, text="NO WHITESPEOPLE!", fg="RED", font=("Courier", 16))
            warning.place(relx=0.25, rely=0.16)
        else:
            warning = Label(app, text="NO CYMBALS!", fg="RED", font=("Courier", 16))
            warning.place(relx=0.25, rely=0.16)
