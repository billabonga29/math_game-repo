...
def create_question_window():
    ...
    def create_error_window(error_message):
        error_window = Toplevel(question_window)
    ...
    def input_validator(user_input):
        ...
        if user_input == "":
            ...
            warning = Label(question_window, text="NO BLANKS!", fg="RED", font=("Courier", 16))
        ...
        # Alphabet, Whitespace and Symbols
            ...
            if user_input.isalnum() == True:
                ...
                warning = Label(question_window, text="NO LETTERS!", fg="RED", font=("Courier", 16))
                warning.place(relx=0.25, rely=0.16)
            elif " " in user_input:
                create_error_window("NO WHITESPACE!")
                warning = Label(question_window, text="NO WHITESPACE!", fg="RED", font=("Courier", 16))
                warning.place(relx=0.25, rely=0.16)
            else:
                create_error_window("NO SYMBOLS!")
                warning = Label(question_window, text="NO SYMBOLS!", fg="RED", font=("Courier", 16))
                warning.place(relx=0.25, rely=0.16)
        # Boundaries (user input is more than)
        elif len(user_input) > 6:
            create_error_window("CHARACTER LIMIT IS 6!")
            warning = Label(question_window, text="CHARACTER LIMIT IS 6", fg="RED", font=("Courier", 16))
            warning.place(relx=0.25, rely=0.16)
        else:
    ...
    def submt(user_entry):
        ...
        if validity == True:
            if user_input == str(resultPLUS()):
                correct = Label(question_window, text="Correct!", fg="green", font=("Courier", 16))
                correct.place(relx=0.3, rely=0.2)
            else:
                wrong = Label(question_window, text="Wrong!!!", fg="red", font=("Courier", 16))
    ...
    def try_again():
        ...
        newQ = Label(
            question_window, text=f"{try_again.num1update}+{try_again.num2update}", font=("Courier", 16)
        )
    ...
    # Question Window
    question_window = Toplevel(app)
    question_window.title("Question Window")
    question_window.geometry("400x400")
    question_window.resizable(False, False)

    # Start Button
    start = Button(question_window, text="Begin Adventure", font=("Times New Roman","10"), command=try_again)
    start.place(relx=0.30, rely=0.1, relheight= 0.17, relwidth = 0.4)
    
    # Entry textbox
    solving = Entry(question_window, font = ("Comic Sans MS", 12))
    ...
    submit = Button(question_window, image=submit_img, command=lambda: submt(solving))
    submit.place(relx=0.32, rely=0.64, relwidth=0.34, relheight=0.14)
    start.image = submit_img  

    # Try Again
    try_again = Button(question_window, text="Try Again", command=try_again)
...
# Start Button
start = Button(app, image = start_img, font=("Times New Roman","10"), command=create_question_window)
start.place(relx=0.30, rely=0.25, relheight= 0.25, relwidth = 0.4)
